...
function init() {
    ...
    fetch('/get-boards')
        ...
        .then(boards => {
            ...
            // Add placeholder option for "Select Board"
            boardSelect.innerHTML = '<option value="" disabled selected>Select Board</option>'; // Add default option

            // Populate the select element with the boards and their fqbn values
            boards.forEach(board => {
                const option = document.createElement('option');
                option.value = board.fqbn;  // Store fqbn in the option value
...
function uploadClick() {
    ...
    fetch("/upload", {
        ...
        body: JSON.stringify({
            code: arduinoCode,
            fqbn: boardFqbn  // Send the selected board's fqbn to the backend
        })
    })
        .then(response => {
            if (response.ok) {
                writeToTerminal("Sketch uploaded. Starting upload process...");
                startUploadStream();
            } else {
                writeToTerminal("Upload failed. Please check the code.");
            }
        })
        .catch(error => {
            writeToTerminal("Error occurred during upload: " + error);
        });

    writeToTerminal("Compiling code...");
}
